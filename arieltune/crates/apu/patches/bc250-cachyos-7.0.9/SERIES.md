...
| `13` | `13-gfxoff-disable-gfx1013.patch` | `gfx_v10_0.c` | **NEW** GFXOFF disabled for gfx1013 — prevents GPU power-state hangs (c) Fabian & Dani (git.fmkr.de/Fabian), used with permission; src git.sudx.de/Fabian/ROCm-Research-Archive |
| `14` | `14-gmc-kiq-bypass-dead-gpu.patch` | `gmc_v10_0.c` | **NEW** KIQ bypass + dead-GPU detection in gmc_v10_0 TLB flush (5 sub-patches) (c) Fabian & Dani (git.fmkr.de/Fabian), used with permission; src git.sudx.de/Fabian/ROCm-Research-Archive |
| `15` | `15-amdgpu-gmc-kiq-bypass.patch` | `amdgpu_gmc.c` | **NEW** KIQ bypass + dead-GPU detection in centralized GMC code (2 sub-patches) (c) Fabian & Dani (git.fmkr.de/Fabian), used with permission; src git.sudx.de/Fabian/ROCm-Research-Archive |
...
| `26` | `26-bc250-sdma-firmware-override.patch` | `amdgpu_sdma.c` | **APPLIED** SDMA firmware override — the cyan_skillfish2 blob never drives user queues (GabriWar docs/28+29); navi10/navi12 blobs work. Gated by `amdgpu.bc250_sdma_fw=<base>`. Validation path to retiring patch 19. |
| `27` | `27-bc250-early-sdma-trap.patch` | `sdma_v5_0.c` | **APPLIED** Write SDMA TRAP_ENABLE in gfx_resume — removes the two 500 ms "Fence fallback" stalls at boot. Companion to patch 26. |
...
| `29` | `29-bc250-tmr-discovery-offset-fix.patch` | `amdgpu_discovery.c`, `amdgpu_discovery.h` | **NEW** Honor the IFWI-reported discovery TMR location: probe the legacy VRAM default first, then take size/offset from `mmDRIVER_SCRATCH_0/1/2`, with sysmem support. Backport of the upstream `get_tmr_info` logic (newer kernels) — fixes `invalid ip discovery binary signature` on boards whose firmware does not place the TMR at the legacy offset. |
| `30` | `30-cyan-skillfish2-hardcoded-fallback.patch` | `amdgpu_discovery.c` | **NEW** If discovery is still unavailable (TMR never populated by some IFWI/UMA combinations), fall back to the pre-discovery hardcoded cyan skillfish IP table instead of failing the probe. Compile-validated; the fallback only executes when discovery fails. |
